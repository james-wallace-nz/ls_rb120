...
class Player
  ...
  def get_name(player_type)
    player_name = nil
    loop do
      puts "Enter a name for #{player_type}:"
...
class TTTGame
  ...
  attr_accessor :current_player, :first_to_move, :scores

  def initialize
    @board = Board.new
    set_human_marker
    @human = Player.new(@human_marker, 'yourself')
    @computer = Player.new(COMPUTER_MARKER, 'the computer')
    @scores = {
      @human_marker => 0,
      COMPUTER_MARKER => 0
    }
  ...
  def display_match_scores
    puts  "#{@human.name} has #{scores[@human_marker]} and #{@computer.name} has #{scores[COMPUTER_MARKER]}. " \
  ...
  def display_match_result
    ...
    if winners.size == 2
      ...
      puts "#{@human.name} won! #{human_score} to #{computer_score}."
    elsif winners.first == COMPUTER_MARKER
      puts "#{@computer.name} won! #{computer_score} to #{human_score}."
    end
  end
