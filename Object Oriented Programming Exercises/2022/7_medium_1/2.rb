...
class FixedArray
  def initialize(length)
    @fixed_array = create_new_fixed_array(length)
  end

  def [](idx)
    # raise IndexError unless idx >= -fixed_array.length && idx < fixed_array.length
  ...
  def to_s
    # fixed_array.to_a.to_s
    to_a.to_s # calling to_s on the return value of to_a
  end
  ...
  def create_new_fixed_array(length)
    # new_array = []
    # length.times do
    #   new_array << nil
    # end
